...
<div class="row">
    ...
    @php
      ...
      $userUnpaidLeaves = app('App\Http\Controllers\Admin\LeaveRequestController')->getTotalLOPCount();
    @endphp
      ...
      <div class="card att-statistics mx-auto" style="padding: 20px; margin: 30px 0;">
          <div class="card-body">
              ...
              <div class="stats-list">
                  ...
                  <!--</div>-->
                  <div class="stats-info">
                      <p style="margin-right: 111px;">LOP's </p><strong>{{$userUnpaidLeaves}} </strong>
                  </div>
              </div>
          </div>
      </div>  
                    ...
                    @foreach ($leaveRequests as $leaveRequest)
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="revoke-btn">{{ $leaveRequest->status }}</div>
                            </td>                            
                        </tr>
                    @endforeach
                </tbody>
...
<div id="revoke_leave" class="modal custom-modal fade" role="dialog">
	<div class="modal-dialog modal-dialog-centered" role="document">
        ...
        {{-- <form>
          ...
          {{-- <form id="revoke-form" method="POST" action="{{ route('leave-request.revoke', ['id' => $leaveRequest->id]) }}" data-leave-id="{{ $leaveRequest->id }}"> --}}
        {{-- </form>         --}}
        <form action="{{ route('leave-request.revoke') }}" method="POST">   
          @csrf
          @method('PUT')
          <div class="form-group">
            <label>Leave Revoke Reason <span class="text-danger">*</span></label>
          ...
          <!-- Add a hidden input field to store the leave ID -->
...
{{-- <script>
  const revokeBtns = document.querySelectorAll('.revoke-btn button');

  revokeBtns.forEach((btn) => {
    btn.addEventListener('click', () => {
      const leaveId = btn.getAttribute('data-leave-id');
      // Set the value of the form's data-leave-id attribute to the leave ID
      const revokeForm = document.getElementById('revoke-form');
      revokeForm.setAttribute('data-leave-id', leaveId);
      // Update the value of the hidden input field
      const leaveIdInput = revokeForm.querySelector('input[name="leave_id"]');
...
$(document).ready(function() {
    $('#leave_type').change(function() {
        ...
        $.ajax({
            url: '/leave-type/' + leaveTypeId + '/days-available',
            type: 'GET',
  ...
  $(document).ready(function() {
      $('#leave_type').change(function() {
          ...
          $.ajax({
              ...
              type: 'GET',
              success: function(data) {
                  if (data.days_available === null) {
                      $('#days_available').html('No limit for this leave type');
                  } else {
                      $('#days_available').html(data.days_available + ' leaves available');
                  }
              }
